...
другой результат. Остаётся нетронутым только маршрут 3 (затухание бевела
у остриёв на всех семи `_SYNTH_BEVEL`-курсорах, `STATUS.md` "Симметрия и
освещение") - решение владельца, отдельная задача, не расширение этого
эксперимента.
