...
силу. Внутренняя грань лежит в той же области, что и переход складки, поэтому
пространственная маска этих двух требований не разводит.
...
**Архитектурный вывод.** Конфликт модельный, а не в распределении владения. У
`_tip_relight` две плоские грани и зарубка, а нарисовано там больше. Следующая
модель должна сама уметь представить одновременно широкий переход и отдельную
внутреннюю грань со своим остриём. Возвращаться к `temper = 1` - глобальному или
зонному - без такой модели не надо.
