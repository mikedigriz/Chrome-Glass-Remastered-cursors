...
| Анимация | 9 кадров, ~20 fps | **27 кадров, 60 fps** (Windows/macOS) / родные ~20 fps (Linux), авторский ритм сохранён |
...
> **Курсор мигает?** С переходом Linux-анимаций на ~20 fps этого быть не должно. Курсор ожидания и рука над ссылкой - анимированные, а перерисовка анимированного курсора в Xcursor не синхронизирована с обновлением экрана, поэтому на прежних 60 fps смена кадра периодически попадала не в такт с 60-герцовой панелью и читалась как мигание. Статичные курсоры не меняются, у них такого никогда не было. Если казалось, что баг только в браузере - просто в браузере вы весь день сидите на анимированной руке, а в файловом менеджере почти всегда статичная стрелка.
>
> Мигает и после обновления темы? **Сначала перезапустите приложение.** Курсоры кэшируются при старте, и браузер, запущенный до обновления, продолжает показывать старые.
