...
Курсор собирается из трёх слоёв: **оригинал 32 px** - для подлинности; **AI-апскейл до 512 px**, посчитанный один раз и закоммиченный в репозиторий - даёт цвет и переливы на всех размерах (ужать чище, чем растянуть); и **векторный контур** - чтобы края были чёткими при любом масштабе. Даже бледные, почти серые курсоры (Help, IBeam, Cross, стрелки resize) получают цвет от нейросети: апскейлер настроен под чистую иллюстрацию и не сыплет шумом на плоское серое стекло, а отдельный проход резкости добавляет чёткие кромки без лишней текстуры.

Прозрачность апскейлится так же, но отдельно от цвета - если растянуть её прямо с 32 px, стеклянное свечение размывается в кашу. Цвета в альфа-канале нет, портить нечего, поэтому увеличенную версию используют все курсоры, включая бледные.
...
Пара мелочей нарисована вручную в `cursors.py`, а не найдена трассировщиком - например, точка под `?` у Help: она стоит отдельно от стрелки, и автоматика её не видит.
...
Именно в таком порядке: мастера цвета строятся из базы 128 px, мастер альфы - из альфы оригинала. Нужен один файл весов - `RealESRGAN_x4plus_anime_6B.pth` (~18 МБ, заточен под иллюстрации), положите его в `weights/` сами (`upscale_lib.load_model` грузит локально, без скачивания). Результаты уже закоммичены, поэтому остальным этим заниматься не нужно.
...
Chrome Glass годами остаётся моим любимым набором курсоров - спасибо yoyos. Этот репозиторий - попытка вдохнуть в него новую жизнь.
