...
В 2006 году на DeviantArt появился набор курсоров «Chrome Glass» - полупрозрачный, переливающийся, живой. На современных экранах его 32-пиксельная графика превратилась в мутные кляксы - я вернул её к жизни. **Это тот самый набор, а не подражание**: оригинал идёт байт в байт, а всё вокруг пересобрано под нынешние разрешения.

![оригинал против ремастера на HiDPI](assets/comparison.png)

| | Chrome Glass (2006) | Chrome Glass Remastered |
|---|---|---|
| Разрешение | 32 px | **32-256 px**, векторные кромки без растрового мыла |
...
Каждый курсор - гибрид трёх источников: оригинальные кадры 32 px (`src/orig/`) дают родную стеклянную полупрозрачность, AI-апскейл 128 px (`src/ai/`) - авторские переливы (цветоперенос Рейнхарда возвращает смытую насыщенность), векторные силуэты (`traced.json`) - чёткую кромку на любом размере. Слой 256 px дополнительно прогнан через Real-ESRGAN (`src/ai256/`); результаты закоммичены, так что сборке torch не нужен.
